...
├── claude.zsh             # Shell 封装（cl/cl_auto 函数）
...
`claude.zsh` 提供两个 Zsh 函数，用于启动自动加载配置的 Claude Code：

- **`cl`** — 正常模式，保留权限提示。自动加载 `system-prompt.txt`、项目 `CLAUDE.md` 和 `~/.claude/` 下的 MCP 配置。
- **`cl_auto`** — 自动模式，使用 `--dangerously-skip-permissions`。配置加载与 `cl` 相同。
