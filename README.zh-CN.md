...
├── agents/                # 自定义 Agent（search）
├── mcp/                   # MCP 服务器配置（Lark + Playwright）
├── plugins/               # 插件安装指南（22 个插件，7 个市场）
├── skills/                # 自定义技能（paper-reading、adversarial-review、humanizer、update_config）
├── claude.zsh             # Shell 封装（cl/cl_auto + GLM 双后端）
├── system-prompt.txt      # Shell 封装的系统提示词
├── glm-env.json           # GLM API 凭据模板
├── docs/                  # 论文阅读总结
├── images/                # 展示截图
├── VERSION                # 语义化版本号
...
```
  ...
  > [*] CLAUDE.md                全局指令模板
    ...
    [*] Search agent             Jeff 只读网络搜索 Agent
    [*] Shell wrapper            cl/cl_auto zsh 函数 + 系统提示词
  ...
  MCP Servers
    [ ] MCP Servers              Lark + Playwright 集成
...
| Agent | 说明 |
|-------|------|
| **search** | 只读网络搜索 Agent（"Jeff"）。仅使用只读/在线工具收集信息，简要总结并附来源。不进行编辑或推测。使用 Opus 模型。 |

自定义 Agent 放在 `agents/<name>.md`。

### Shell 封装

`claude.zsh` 提供多个 Zsh 函数，用于启动自动加载配置的 Claude Code，支持**双后端**（Claude API + GLM API）：

| 命令 | 后端 | 模式 |
|------|------|------|
| **`cl`** | 自动（按配置路由） | 正常（保留权限提示） |
| **`cl_auto`** | 自动（按配置路由） | 自动（`--dangerously-skip-permissions`） |
| **`cl_claude`** | Claude API（强制） | 正常 |
| **`cl_claude_auto`** | Claude API（强制） | 自动 |
| **`cl_glm`** | GLM API（强制） | 正常 |
| **`cl_glm_auto`** | GLM API（强制） | 自动 |
| **`cl_switch`** | — | 查看或切换默认配置（`claude` / `glm`） |

`cl` 和 `cl_auto` 根据 `~/.claude/default-profile` 自动路由到对应后端（默认 `claude`）。使用 `cl_switch glm` 切换默认后端，或使用显式命令（`cl_claude`、`cl_glm`）临时覆盖。

**GLM 配置**：编辑 `~/.claude/glm-env.json` 填入智谱凭据：
...
封装器自动：
- 加载 `system-prompt.txt` 作为追加的系统提示词
- 如果当前项目存在 `CLAUDE.md`，自动追加
- 检测并加载 `~/.claude/mcp_settings.json` 或 `~/.claude/mcp/mcp-servers.json` 中的 MCP 服务器配置
- 从 `settings.json` 中移除 `skipDangerousModePermissionPrompt`（安全保护）
- 使用 GLM 后端时，在调用期间注入 GLM 环境变量
...
然后使用 `cl` 或 `cl_auto` 代替 `claude`。安装器会在安装 Shell 封装时提示选择默认后端（Claude 或 GLM）。
...
- **添加 Agent**：放到 `agents/<name>.md`，包含 YAML frontmatter（`description`、`model`）
