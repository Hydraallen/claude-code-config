...
This skill runs inside Claude Code, so it uses the local filesystem and the standard tools directly:

- **Reading files**: use the `Read` tool. It reads PDFs (text and scanned/rasterized pages), images, and Jupyter notebooks natively. For `.pptx`/`.docx`, use whatever document-processing skill is available, or ask the user to export to PDF first.
- **Output location**: write the deliverable to a `cheatsheet/` folder and the scratch work to a `cheatsheet-work/` folder, both **relative to the current working directory** (or a target directory the user names in Step 0). There is no `/mnt/user-data/outputs/` — use plain local paths.
- **Presenting results**: there is no `present_files` tool. When done, list the produced file paths to the user in your reply so they can open them.
...
- **Output directory.** Where the `cheatsheet/` folder should be written (default: the current working directory).
...
1. **Parse it with the `Read` tool.**
   - Text-based PDF: read it directly.
   - Scanned or handwritten PDF: `Read` rasterizes and shows the pages so they can be read visually. Handwritten math is the most error-prone part of the whole pipeline — go slowly and double-check transcribed formulas.
   - PPTX / DOCX: use an available document-processing skill, or ask the user to export to PDF first.
...
**Output folder structure**: everything goes inside a single `cheatsheet/` folder in the output directory chosen in Step 0 (default: current working directory).
...
**Intermediate folder**: if the user opted in (default), copy the contents of `cheatsheet-work/` into `cheatsheet/intermediate/`. If they opted out, only the two cheatsheet files go in `cheatsheet/`. Either way, list *all* the resulting file paths in your reply so the user can open and review them.
...
When done, list every produced file path in your reply — the concise cheatsheet first (primary deliverable), then the expanded version, then the intermediate files if they were saved.
...
Adapted for Claude Code from [cheatsheet-creator-skill](https://github.com/wyc79/cheatsheet-creator-skill) by wyc79 (MIT License). Changes: filesystem output uses local paths instead of `/mnt/user-data/outputs/`; file parsing uses the `Read` tool instead of the `pdf-reading`/`pptx`/`docx` skills; results are listed as file paths instead of via `present_files`.
